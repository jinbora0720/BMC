...
seedsave = rep(NA, 30)
for (i in 1:30) {
  res <- readRDS(paste0("~/BMC/BMC/data/sim1_BMC_res_", i, ".RDS"))
  rmse[i] <- res$rmse
...
seedsave = rep(NA, 30)
for (i in 1:30) {
  res <- readRDS(paste0("~/BMC/BMC/data/sim1_BMCi_res_", i, ".RDS"))
  rmse[i] <- res$rmse
  tr_aucg[i] <- res$tr_aucg
  tt_aucg[i] <- res$tt_aucg
  tr_auct[i] <- res$tr_auct
...
saveRDS(list(rmse = rmse, tr_aucg = tr_aucg, tt_aucg = tt_aucg, 
             ...
             tr_aucap = tr_aucap, tt_aucap = tt_aucap,
             seedsave = seedsave), 
        "~/BMC/BMC/data/sim1_BMCi_all.RDS")
